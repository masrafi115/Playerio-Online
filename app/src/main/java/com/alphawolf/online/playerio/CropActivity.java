...
public class CropActivity extends  AppCompatActivity  { 
	...
	private StorageReference fbs_user_pic = _firebase_storage.getReference("profile/picture");
	private OnCompleteListener<Uri> _fbs_user_pic_upload_success_listener;
	private OnSuccessListener<FileDownloadTask.TaskSnapshot> _fbs_user_pic_download_success_listener;
	private OnSuccessListener _fbs_user_pic_delete_success_listener;
	private OnProgressListener _fbs_user_pic_upload_progress_listener;
	private OnProgressListener _fbs_user_pic_download_progress_listener;
	private OnFailureListener _fbs_user_pic_failure_listener;
	@Override
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		textview2.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				try {
					...
					if (16 > (FileUtil.getFileLength(file_path) / 1048576)) {
						if (url.length() > 0) {
							user_pic.child(url).removeValue();
						}
						fbs_user_pic.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).putFile(Uri.fromFile(new File(file_path))).addOnFailureListener(_fbs_user_pic_failure_listener).addOnProgressListener(_fbs_user_pic_upload_progress_listener).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
							@Override
							public Task<Uri> then(Task<UploadTask.TaskSnapshot> task) throws Exception {
								return fbs_user_pic.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).getDownloadUrl();
							}}).addOnCompleteListener(_fbs_user_pic_upload_success_listener);
						_prog(true, "Uploading Image!", "Please wait...");
					}
					else {
						SketchwareUtil.showMessage(getApplicationContext(), "Image file must be lower than 15MB bro!");
						_prog(false, "", "");
						isUploading = false;
						finish();
					}
		...
		_user_pic_child_listener = new ChildEventListener() {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				if (_childKey.equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
					url = _childValue.get("avatar").toString();
				}
			}
		...
		_fbs_user_pic_upload_progress_listener = new OnProgressListener<UploadTask.TaskSnapshot>() {
			@Override
			public void onProgress(UploadTask.TaskSnapshot _param1) {
				double _progressValue = (100.0 * _param1.getBytesTransferred()) / _param1.getTotalByteCount();
				_prog(true, "Uploading Image!", String.valueOf((long)(_progressValue)).concat("% ".concat("Uploaded")));
			}
		};
		
		_fbs_user_pic_download_progress_listener = new OnProgressListener<FileDownloadTask.TaskSnapshot>() {
			@Override
		...
		_fbs_user_pic_upload_success_listener = new OnCompleteListener<Uri>() {
			@Override
			public void onComplete(Task<Uri> _param1) {
				final String _downloadUrl = _param1.getResult().toString();
				map = new HashMap<>();
				map.put("avatar", _downloadUrl);
				user_pic.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).updateChildren(map);
				FileUtil.deleteFile(path_file);
				_prog(false, "", "");
				isUploading = false;
				SketchwareUtil.showMessage(getApplicationContext(), "Profile Picture Uploaded Successfully!");
				data.edit().putString("button", "1").commit();
				finish();
			}
		};
		
		_fbs_user_pic_download_success_listener = new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
			@Override
		...
		_fbs_user_pic_delete_success_listener = new OnSuccessListener() {
			@Override
		...
		_fbs_user_pic_failure_listener = new OnFailureListener() {
			@Override
	...
	private void initializeLogic() {
		...
		_setBackground(textview2, 10, 0, "#212121", true);
		_setBackground(textview1, 10, 0, "#212121", true);
		file_path = FileUtil.getPackageDataDir(getApplicationContext()).concat("/musicgarage_user_pic.png");
		...
		Window window = this.getWindow();window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);  window.setNavigationBarColor(Color.parseColor("#212121"));
		path_file = data.getString("result", "");
		data.edit().remove("result").commit();
		...
		textview2.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/opensansregular.ttf"), 0);
	}
